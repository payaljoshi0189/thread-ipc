...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    mutexIn: Mutex = new Mutex				--Mutex for Producer
    mutexOut: Mutex = new Mutex				--Mutex for Consumer		
    mutexPrint: Mutex = new Mutex			--Mutex for Print	
    semaphoreProducer: Semaphore = new Semaphore	--Semaphore for producer
    semaphoreConsumer: Semaphore = new Semaphore	--Semaphore for Consumer
    empty_buffs: int = 5				--Count variable of producer semaphore
    full_buffs: int = 0					--Count variable of consumer semaphore
 
  function ProducerConsumer ()
   -- Initializing the semaphore and mutex
      mutexIn.Init()
      mutexOut.Init()
      mutexPrint.Init()
      semaphoreProducer.Init(empty_buffs)
      semaphoreConsumer.Init(full_buffs)
  ...
  function Producer (myId: int)
   -- This method is uses semaphore and checks if the buffer is full
   -- If yes, then it waits  else produces a character and adds it to the buffer
   -- Then it calls Up() on cosumer semaphore to wake it up.

       
      for i = 1 to 5
        semaphoreProducer.Down()
	...
	-- Add c to the buffer
        ...
        PrintBuffer (c)
	mutexPrint.Unlock()
	semaphoreConsumer.Up()
	mutexIn.Unlock()
      endFor
    endFunction

  function Consumer (myId: int)
   -- This method call Down() method of semaphore. If the count of semaphore goes negative, 
   -- it waits and wakes up again after the producer calls Up() on it
   -- If the call to Down() succeeds without going negative, then consumer consumes characters from
   -- from buffer and calls Up() on producer.

      var
        c: char
      while true
        semaphoreConsumer.Down()
...
-- This class is used to implement a monitor to control synchronization
-- between the philosopher threads.
-- The class declares following variables:
--    status array:
--	It maintains the status of philosopher threads.Status can be 
--	THINKING,EATING and HUNGRY
--    monitormutex:
--	This mutex is used to protect the monitor to ensure only one thread is 
--	executed within the monitor
--    condition array:
--	This array is of type Condition.Each philosopher will have its own
--	Condition variable
-- The class provides following methods:
--    Init()
--	This method initializes the status array so that all the philosophers
--	are in THINKING state. It also initializes the mutex that is used to protect the
--	monitor and the Condition array.
--    PrintAllStatus()
...
--    PickupForks(int p)
--	This method is called when Philosopher 'p' wants to eat. As a first step, it acquires the mutex lock
--      to make sure only one philosopher can execute this method.
--	This method sets the status of the philosopher to HUNGRY and tests if its LEFT or
--	RIGHT neighbors are not eating,if not then changes 'p''s status to EATING and signals it.
--	If any of its neighbour is EATING, then it calls Wait() on the philosopher and releases the mutex lock.
--    PutDownForks(int p)
--	This method is called when philosopher is done eating.As a first step, it acquires the mutex lock
--	to make sure only one philosopher can execute this method.It sets the status of the 'p' philosop
--      her to THINKING and tests first if its LEFT neighbour is hungry and if LEFT's neighbours are
-- 	not eating,if this is true then sets its status to EATING and signals the LEFT neighbours
--	If the LEFT neighbour does not qualify after testing, then it tests the RIGHT neighbour.
--	It then releases the mutex lock.
--    Test(int p)
--      This method tests if the philosopher p is in HUNGRY state and its left and right
--      neighbours are not eating. If true then change its state to be
--      EATING and signal the philosopher p
  ...
  behavior ForkMonitor
   ...
   method Test (p: int)
      var
       LEFT: int = (p + 1) % 5 
       RIGHT: int = (p - 1) % 5
       while(status[p] == HUNGRY && status[LEFT] != EATING && status[RIGHT] != EATING)
        status[p] = EATING
        mon. PrintAllStatus ()
        condition[p].Signal(&monitormutex)
       endWhile
   endMethod
